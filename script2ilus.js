...
// Funció per carregar dades des de CSV
...
// Dades d'exemple com a fallback
  ...
  years.forEach(year => {
    tabsContainer.append("div")
      ...
      .on("click", () => {
        ...
        // Resetear tots els filtres al canviar d'any
        resetAllFilters();
...
function updateFilters() {
  console.log("Actualitzant filtres");
  ...
  filterFields.forEach(({ id, field }) => {
    ...
    // Netejar opcions anteriors excepte la primera (buida)
...
function updateStats() {
  ...
  if (Object.keys(selectedFilters).length > 0) {
    // Filtrar dades segons seleccions
  ...
  if (yesCountElement) {
    yesCountElement.textContent = percentage;
  }
}

function renderPictograms(data) {
  console.log("Renderitzant pictogrames");
  ...
  const labelX = columns * (size + horizontalSpacing) + 30; // Posició de les etiquetes
  ...
  // Calcular total de dades
  const totalYes = d3.sum(yesData, d => d.Valor);

  if (Object.keys(selectedFilters).length === 0) {
    // Sense filtres: mostrar tots els pictogrames normalment
    console.log("Sin filtros seleccionados - mostrando todos");
    
    // Calcular proporció per sexe
    const sexCounts = d3.rollup(yesData, v => d3.sum(v, d => d.Valor), d => d.Sexe);
    
    // Crear 100 pictogrames segons proporció de sexes
    const pictos = [];
    let pictogramId = 0;
    ...
    // Completar fins a 100 si és necessari
    ...
    if (isInitialLoad) {
      ...
      // Sense transició especial quan no hi ha filtres
      picto.attr("transform", (d, i) => {
        const col = i % columns;
        const row = Math.floor(i / columns);
        return `translate(${col * (size + horizontalSpacing)}, ${row * (size + spacing)})`;
    ...
    // Amb filtres: crear pictogrames segons les dades filtrades
    ...
    // Crear pictogrames basats en les dades FILTRADES
    ...
    // Crear tots els pictogrames (inclosos + exclosos)
    const allSexCounts = d3.rollup(yesData, v => d3.sum(v, d => d.Valor), d => d.Sexe);
    const pictos = [];
    let pictogramId = 0;

    // Primer crear els inclosos segons les dades filtrades
    ...
    // Després crear els exclosos per completar fins a 100
    const excludedCount = 100 - includedPictos.length;
    const excludedPictos = [];
    
    // Per als exclosos, utilitzar la proporció original completa
    ...
    // Ajustar exclosos si és necessari
    ...
    // Combinar tots els pictogrames
    const allPictos = [...includedPictos, ...excludedPictos];

    // Dibuixar pictogrames
    const group = svg.append("g").attr("transform", "translate(25, 20)");

    // Afegir etiquetes laterals per filtres actius
    if (totalIncluded > 0) {
      // Etiqueta amb el valor (bold, 16px)
      group.append("text")
        .attr("x", labelX)
        .attr("y", 30)
      ...
      // Etiquetes amb els noms de variables (12px, gris)
    ...
    // Transició de filtre amb dispersió i GIFs
    picto.attr("transform", (d, i) => {
      const col = i % columns;
      const row = Math.floor(i / columns);
      return `translate(${col * (size + horizontalSpacing)}, ${row * (size + spacing)})`;
    ...
    // Fase 1: Dispersió amb GIFs animats
    setTimeout(() => {
      picto.each(function(d, i) {
        const element = d3.select(this);
        
        // Dispersar tots aleatòriament
        const randomX = Math.random() * width - 100;
        const randomY = Math.random() * 300 - 50;
        element.attr("transform", `translate(${randomX}, ${randomY})`);
        
        // Només els descartats mostren GIFs animats
        if (!d.included) {
      ...
      // Fase 2: Reagrupar en posicions finals
      setTimeout(() => {
        ...
        picto.each(function(d, i) {
          ...
          if (d.included) {
            ...
            // Eliminar GIF animat i restaurar imatge original
            setTimeout(() => {
  ...
  // Ajustar alçada del SVG
  const minHeight = 350;
  const contentHeight = Math.max(minHeight, 500);
  d3.select("#chart").attr("height", contentHeight);

  console.log("Pictogrames renderitzats");
}

// Funció principal d'inicialització
async function init() {
  // Només inicialitzar si no s'ha fet ja
  if (appInitialized) {
    console.log("App ja inicialitzada, renderitzant amb dades existents");
    // Si ja està inicialitzada, només renderitzar amb les dades actuals
  ...
  try {
    ...
    if (currentData.length > 0) {
      ...
      console.error("No es van poder carregar les dades");
    }
  } catch (error) {
...
// NO executar automàticament la inicialització
// Només executar quan es completi la introducció
console.log("Script carregat, esperant que termini la introducció");

// Només executar si D3 està disponible i NO hi ha introducció activa
if (typeof d3 !== 'undefined') {
  console.log("D3 disponible");
  
  // Verificar si la introducció està activa
  if (!document.body.classList.contains('intro-active')) {
    // Si no hi ha introducció, inicialitzar immediatament
  ...
  // Si hi ha introducció, init() no s'executa automàticament
} else {
